...
// initialization: read from checkpoint
    ...
    LlamaWeights(LlamaConfig config, MemorySegment memorySegment) {
        ...
        position[0] += (config.getSeq_len() * config.head_size / 2) * Float.BYTES; // skip what used to be freq_cis_real (for RoPE)
        position[0] += (config.getSeq_len() * config.head_size / 2) * Float.BYTES; // skip what used to be freq_cis_imag (for RoPE)
